...
object t_event_info_copy {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val frame_es = sparkSession
      ...
      .format("es")
      .options(options)
      .load("zz21_vw_issues").persist(StorageLevel.MEMORY_AND_DISK) //zz21_vw_issuehastypes
    val frame_es2 = sparkSession
      .read
      .format("es")
      .options(options)
      .load("zz21_vw_issuehastypes").persist(StorageLevel.MEMORY_AND_DISK)
    println("加载ES完成=============================================》")

    val frame_mysql = sparkSession.read.jdbc(url1, "t_region_vw_code", prop).persist(StorageLevel.MEMORY_AND_DISK)
    val frame_mysql2 = sparkSession.read.jdbc(url1, "t_region_vw_code", prop).persist(StorageLevel.MEMORY_AND_DISK)
    println("===========================================>加载MySQL完成！")
    //定义表结构 10个字段
    ...
    val mysql_cols2 = frame_mysql2.columns
    //新表
    ...
    val frame_copy = frame_mysql.select(mysql_cols.map(x => frame_mysql(x).as("my2_" + x)): _*).select("my2_code", "my2_name", "my2_parent_code")
    //关联
    val frame_res = frame1.join(frame, frame1("ES_OCCURORG") === frame("my_vw_id"), "left")
      .join(frame2, frame1("ES_ID") === frame2("ES2_ISSUEID"), "left")
      .join(frame_copy, frame("my_parent_code") === frame_copy("my2_code"), "left")
    println("=======================================>关联完成")
    ...
    val newRdd: RDD[Row] = frame_res.rdd.mapPartitions(iter => {
      ...
      while (iter.hasNext) {
        ...
        //-------------------------------------------------------
        //val str_code: String = Option(row.getAs[String]("my2_code")).getOrElse("").toString
        val str4: String = Option(row.getAs[String]("my2_name")).getOrElse("").toString
